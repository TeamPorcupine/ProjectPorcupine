...
function MiningDroneStation_JobComplete(job)
    if (job.furniture.Jobs.GetSpawnSpotTile().Inventory == nil or job.furniture.Jobs.GetSpawnSpotTile().Inventory.ObjectType == job.furniture.Parameters["mine_type"].ToString()) then
        World.Current.inventoryManager.PlaceInventory(job.furniture.Jobs.GetSpawnSpotTile(), Inventory.__new(job.furniture.Parameters["mine_type"].ToString() , 50, 2))
    else
        job.CancelJob()
    end
end
...
function MetalSmelter_JobWorked(job)
    job.CancelJob()
    local spawnSpot = job.furniture.Jobs.GetSpawnSpotTile()
...
function PowerCellPress_UpdateAction(furniture, deltaTime)
    ...
    if(spawnSpot.Inventory == nil) then
        if(furniture.JobCount() == 0) then
            ...
            job.RegisterJobCompletedCallback("PowerCellPress_JobComplete")
            job.JobDescription = "job_power_cell_fulling_desc"
            furniture.Jobs.Add(job)
        end
    else
...
function CloningPod_JobComplete(job)
    World.Current.CreateCharacter(job.furniture.Jobs.GetSpawnSpotTile())
...
function PowerGenerator_JobComplete(job)
    job.furniture.Parameters["burnTime"].SetValue(job.furniture.Parameters["burnTimeRequired"].ToFloat())
    job.furniture.PowerConnection.OutputRate = 5
end
