...
public class BuildModeController
{
    ...
    public void DoBuild(Tile t)
    {
        ...
        else if (buildMode == BuildMode.UTILITY)
        {
            ...
            if ( 
                WorldController.Instance.world.IsFurniturePlacementValid(utilityType, t) &&
                DoesBuildJobOverlapExistingBuildJob(t, utilityType) == false)
            {
                ...
                if (WorldController.Instance.world.furnitureJobPrototypes.ContainsKey(utilityType))
                {
                    // Make a clone of the job prototype
                    j = WorldController.Instance.world.furnitureJobPrototypes[utilityType].Clone();
                ...
                for (int x_off = t.X; x_off < (t.X + WorldController.Instance.world.furniturePrototypes[utilityType].Width); x_off++)
                {
                    for (int y_off = t.Y; y_off < (t.Y + WorldController.Instance.world.furniturePrototypes[utilityType].Height); y_off++)
                    {
                        // FIXME: I don't like having to manually and explicitly set
                        // flags that preven conflicts. It's too easy to forget to set/clear them!
                        Tile offsetTile = WorldController.Instance.world.GetTileAt(x_off,y_off);
