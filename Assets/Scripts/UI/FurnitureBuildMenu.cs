...
public class FurnitureBuildMenu : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (string s in World.current.furniturePrototypes.Keys)
        {
            ...
            go.transform.GetComponentInChildren<TextLocalizer>().formatValues = new string[] 
            {
                LocalizationTable.GetLocalization(World.current.furniturePrototypes[s].localizationCode)
            };

            go.GetComponent<Button>().onClick.AddListener(() => bmc.SetMode_BuildFurniture(objectId));
        }
