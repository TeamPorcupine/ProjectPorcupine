...
namespace DiffMatchPatch
{
   ...
   */
    public static class Patcher
    {
        ...
        public const float DiffTimeout = 1.0f;

        // Cost of an empty edit operation in terms of edit characters.
        public const short DiffEditCost = 4;
        ...
        public const int MatchDistance = 1000;

        // When deleting a large block of text (over ~64 characters), how close
        // do the contents have to be to match the expected contents. (0.0 =
        // perfection, 1.0 = very loose).  Note that MatchThreshold controls
        // how closely the end points of a delete need to match.
        public const float PatchDeleteThreshold = 0.5f;

        // Chunk size for context length.
        public const short PatchMargin = 4;
     ...
     */
        public static List<Diff> DiffMain(string text1, string text2, bool checklines)
        {
     ...
     */
        public static Object[] PatchApply(List<Patch> patches, string text)
        {
            ...
            foreach (Patch patch in patches)
            {
                ...
                string text1 = DiffText1(patch.diffs);
                int start_loc;
                int end_loc = -1;
                if (text1.Length > MatchMaxBits)
                {
                    ...
                    start_loc = MatchMain(
                        text, text1.Substring(0, MatchMaxBits), expected_loc);
                    if (start_loc != -1)
                    {
                        end_loc = MatchMain(
                            text,
                            text1.Substring(text1.Length - MatchMaxBits),
                            expected_loc + text1.Length - MatchMaxBits);
                ...
                else
                {
                    ...
                    if (end_loc == -1)
                    {
                        text2 = text.JavaSubstring(
                            start_loc, Math.Min(start_loc + text1.Length, text.Length));
                    }
                    else
                    {
                        text2 = text.JavaSubstring(start_loc, Math.Min(end_loc + MatchMaxBits, text.Length));
                    }

                    if (text1 == text2)
                    {
                        // Perfect match, just shove the Replacement text in.
                        text = text.Substring(0, start_loc) + DiffText2(patch.diffs)
                        ...
                        else
                        {
                            ...
                            foreach (Diff diff in patch.diffs)
                            {
                                if (diff.operation != Operation.EQUAL)
                                {
                                    ...
                                    {
                                        // Insertion
                                        text = text.Insert(start_loc + index2, diff.text);
                                    }
                                    else if (diff.operation == Operation.DELETE)
                                    {
                                        // Deletion
                                        text = text.Remove(
                                            start_loc + index2, DiffxIndex(diffs, index1 + diff.text.Length) - index2);
                                    }
                                }
     ...
     * @return List of Diff objects.
     */
        private static List<Diff> DiffMain(string text1, string text2, bool checklines, DateTime deadline)
        {
     ...
     * @return List of Diff objects.
     */
        private static List<Diff> DiffCompute(string text1, string text2, bool checklines, DateTime deadline)
        {
        ...
        * @return List of Diff objects.
        */
        private static List<Diff> DiffLineMode(string text1, string text2, DateTime deadline)
        {
            ...
            // Upon reaching an equality, check for prior redundancies.
                        if (count_delete >= 1 && count_insert >= 1)
                        {
                            ...
                            List<Diff> a =
                                DiffMain(text_delete, text_insert, false, deadline);
     ...
     * @return List of Diff objects.
     */
        private static List<Diff> DiffBisect(string text1, string text2, DateTime deadline)
        {
     ...
     */
        private static List<Diff> DiffBisectSplit(string text1, string text2, int x, int y, DateTime deadline)
        {
     ...
     */
        private static object[] DiffLinesToChars(string text1, string text2)
        {
     ...
     */
        private static string DiffLinesToCharsMunge(string text, List<string> lineArray, Dictionary<string, int> lineHash)
        {
            int lineStart = 0;
            int lineEnd = -1;
            string line;
     ...
     * @param lineArray List of unique strings.
     */
        private static void DiffCharsToLines(ICollection<Diff> diffs, List<string> lineArray)
        {
            StringBuilder text;
     ...
     */
        private static string[] DiffHalfMatchI(string longtext, string shorttext, int i)
        {
     ...
     * @param diffs List of Diff objects.
     */
        private static void DiffCleanupEfficiency(List<Diff> diffs)
        {
            bool changes = false;
     ...
     * @param diffs List of Diff objects.
     */
        private static void DiffCleanupMerge(List<Diff> diffs)
        {
            // Add a dummy entry at the end.
            diffs.Add(new Diff(Operation.EQUAL, string.Empty));
            int pointer = 0;
     ...
     */
        private static int DiffxIndex(List<Diff> diffs, int loc)
        {
            int chars1 = 0;
            int chars2 = 0;
            int last_chars1 = 0;
     ...
     * @return Source text.
     */
        private static string DiffText1(List<Diff> diffs)
        {
     ...
     */
        private static string DiffText2(List<Diff> diffs)
        {
            StringBuilder text = new StringBuilder();
            foreach (Diff diff in diffs)
            {
     ...
     */
        private static int DiffLevenshtein(List<Diff> diffs)
        {
            int levenshtein = 0;
            int insertions = 0;
            int deletions = 0;
     ...
     */
        private static int MatchMain(string text, string pattern, int loc)
        {
            // Check for null inputs not needed since null can't be passed in C#.
     ...
     */
        private static int MatchBitap(string text, string pattern, int loc)
        {
            ...
            if (best_loc != -1)
            {
                score_threshold = Math.Min(MatchBitapScore(0, best_loc, loc, pattern), score_threshold);

                // What about in the other direction? (speedup)
                best_loc = text.LastIndexOf(
                    pattern,
                ...
                if (best_loc != -1)
                {
                    score_threshold = Math.Min(
                            MatchBitapScore(0, best_loc, loc, pattern), score_threshold);
                }
            }
     ...
     */
        private static List<Patch> PatchDeepCopy(List<Patch> patches)
        {
     ...
     */
        private static string PatchAddPadding(List<Patch> patches)
        {
     ...
     */
        private static void PatchSplitMax(List<Patch> patches)
        {
            short patch_size = MatchMaxBits;
            for (int x = 0; x < patches.Count; x++)
            {
                ...
                while (bigpatch.diffs.Count != 0)
                {
                    ...
                    while (bigpatch.diffs.Count != 0
                           && patch.length1 < patch_size - PatchMargin)
                    {
                        ...
                        else
                        {
                            ...
                            diff_text = diff_text.Substring(
                                0, Math.Min(diff_text.Length, patch_size - patch.length1 - PatchMargin));
                            patch.length1 += diff_text.Length;
                            start1 += diff_text.Length;
                    ...
                    precontext = precontext.Substring(
                        Math.Max(0, precontext.Length - PatchMargin));
     ...
     * @param diffs List of Diff objects.
     */
        private static void DiffCleanupSemantic(List<Diff> diffs)
        {
            bool changes = false;
                    ...
                    diffs[pointer].operation == Operation.INSERT)
                {
                    ...
                    if (overlap_length1 >= overlap_length2)
                    {
                            ...
                            overlap_length1 >= insertion.Length / 2.0)
                        {
                            ...
                            diffs.Insert(pointer, new Diff(Operation.EQUAL, insertion.Substring(0, overlap_length1)));
                            diffs[pointer - 1].text =
     ...
     * @param diffs List of Diff objects.
     */
        private static void DiffCleanupSemanticLossless(List<Diff> diffs)
        {
            int pointer = 1;
