...
/// <summary>
/// Utilities are things like wires and pipes.
/// </summary>
[MoonSharpUserData]
public class Utility : IXmlSerializable, ISelectable, IContextActionProvider, IPowerRelated
{
    ...
    /// <summary>
    /// These actions are called every update. They get passed the utility
    /// they belong to, plus a deltaTime.
    /// </summary>
    // protected Action<Utility, float> updateActions;
    ...
    // private Func<Utility, ENTERABILITY> IsEnterable;
    ...
    // If this utility gets worked by a person,
    ...
    // utility tile itself!  (In fact, this will probably be common).
    ...
    public ENTERABILITY IsEnterable()
    {
        ...
        //// UtilityActions.CallFunctionsWithUtility( isEnterableActions.ToArray(), this );
    ...
    public List<string> ReplaceableUtility
    {
        get
        {
            return replaceableUtility;
        }
    }
    ...
    // Make a copy of the current utility.  Sub-classed should
    ...
    protected bool DEFAULT__IsValidPosition(Tile t)
    {
        ...
        for (int x_off = t.X; x_off < (t.X + Width); x_off++)
        {
            for (int y_off = t.Y; y_off < (t.Y + Height); y_off++)
            {
                ...
                // Make sure tile doesn't already have utility
    ...
    public void Deconstruct()
    {
        ...
        {
            Utility u = tile.Utility;
            uwidth = u.Width;
            uheight = u.Height;
            linksToNeighbour = u.linksToNeighbour;
            u.CancelJobs();
        }
        ...
        {
            for (int xpos = x - 1; xpos < (x + uwidth + 1); xpos++)
            {
                for (int ypos = y - 1; ypos < (y + uheight + 1); ypos++)
                {
    ...
    // Returns true if utility has typeTag, though simple, the intent is to separate the interaction with
