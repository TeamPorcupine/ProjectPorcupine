...
/// <summary>
/// Utility Objects such as pipes and wires.
/// </summary>
[MoonSharpUserData]
public class Utility : IXmlSerializable, ISelectable, IContextActionProvider, IPowerRelated
{
    private float powerValue;
    /// <summary>
    /// Custom parameter for this particular utility object.  We are
    ...
    /// </summary>
    protected Parameter utilParameters;

    /// <summary>
    /// These actions are called every update. They get passed the utilility
    ...
    /// <summary>
    /// These actions are called when an object is installed. They get passed the utility and a delta
    /// time of 0
    /// </summary>
    protected List<string> installActions;

    /// <summary>
    /// These actions are called when an object is uninstalled. They get passed the utility and a delta
    /// time of 0
    /// </summary>
    ...
    protected List<string> replaceableUtility = new List<string>();

    private List<Job> jobs;

    // If this utilility gets worked by a person,
    ...
    // utilility tile itself!  (In fact, this will probably be common).
    ...
    public void Update(float deltaTime)
    {
        if (updateActions != null)
        {
            ...
            UtilityActions.CallFunctionsWithUtility(updateActions.ToArray(), this, deltaTime);
        }
    }
    ...
    public string GetSpriteName()
    {
        ...
        DynValue ret = UtilityActions.CallFunction(getSpriteNameAction, this);
        return ret.String;
    }

    // If this utility generates power then powerValue will be positive, if it consumer power then it will be negative
    ...
    public event Action<Utility> cbOnChanged;

    public event Action<Utility> cbOnRemoved;
    ...
    public Utility()
    {
        ...
        utilParameters = new Parameter("utilParameters");
        jobs = new List<Job>();
    ...
    protected Utility(Utility other)
    {
        ...
        this.utilParameters = new Parameter(other.utilParameters);
        jobs = new List<Job>();
    ...
    // Make a copy of the current utilility.  Sub-classed should
    ...
    static public Utility PlaceInstance(Utility proto, Tile tile)
    {
        ...
        {
            // This type of utilility links itself to its neighbours,
            ...
            for (int xpos = x - 1; xpos < (x + proto.Width + 1); xpos++)
            {
                for (int ypos = y - 1; ypos < (y + proto.Height + 1); ypos++)
                {
                    ...
                    if (t != null && t.Utility != null && t.Utility.cbOnChanged != null)
                    {
                        t.Utility.cbOnChanged(t.Utility);
                    }
                }
            }
        }
        ...
        if(obj.installActions != null )
            UtilityActions.CallFunctionsWithUtility(obj.installActions.ToArray(), obj, 0);

        // Update thermalDiffusifity using coefficient
        float thermalDiffusivity = Temperature.defaultThermalDiffusivity;
        if(obj.utilParameters.ContainsKey("thermal_diffusivity"))
        {
            thermalDiffusivity = obj.utilParameters["thermal_diffusivity"].ToFloat();
        }
    ...
    // LUA files that will be customizable for each piece of utilility.
    ...
    protected bool DEFAULT__IsValidPosition(Tile t)
    {
        ...
        for (int x_off = t.X; x_off < (t.X + Width); x_off++)
        {
            for (int y_off = t.Y; y_off < (t.Y + Height); y_off++)
            {
                ...
                // Check to see if there is utility which is replaceable
                bool isReplaceable = false;

                if (t2.Utility != null)
                {
                    for (int i = 0; i < replaceableUtility.Count; i++)
                    {
                        if (t2.Utility.HasTypeTag(replaceableUtility[i]))
                        {
                            isReplaceable = true;
                        }
                    }
                ...
                // Make sure tile doesn't already have utilility
    ...
    public void ReadXmlPrototype(XmlReader reader_parent)
    {
        ...
        while (reader.Read())
        {
            ...
            case "EnclosesRooms":
                ...
                break;
            case "CanReplaceUtility":
                replaceableUtility.Add(reader.GetAttribute("typeTag").ToString());
                break;
            case "DragType":
                reader.Read();
            ...
            case "BuildingJob":
                ...
                Job j = new Job(
                    null,
                    objectType,
                    UtilityActions.JobComplete_UtilityBuilding,
                    jobTime,
                    invs.ToArray(),
                    Job.JobPriority.High );
                j.JobDescription = "job_build_" + objectType + "_desc";
                World.current.SetUtilityJobPrototype(j, this);
                break;
    ...
    /// <summary>
    /// Gets the utilility's Parameter structure from a string key.
    /// </summary>
    ...
    public void Deconstruct()
    {
        ...
        if (uninstallActions != null)
            UtilityActions.CallFunctionsWithUtility(uninstallActions.ToArray(), this, 0);
    ...
    // Returns true if utilility has typeTag, though simple, the intent is to separate the interaction with
    //  the Utility's typeTags from the implementation.
    ...
    {
        UtilityActions.CallFunction(luaFunction, this, character);
    }
}
