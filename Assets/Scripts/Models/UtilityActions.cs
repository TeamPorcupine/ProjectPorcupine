...
{
    private static UtilityActions _Instance;
    ...
    public static void CallFunctionsWithUtility(string[] functionNames, Utility util, float deltaTime)
    {
        foreach (string ut in functionNames)
        {
            object func = _Instance.myLuaScript.Globals[ut];

            if (func == null)
            {
                Debug.LogError("'" + ut + "' is not a LUA function.");
                return;
            }

            DynValue result = _Instance.myLuaScript.Call(func, util, deltaTime);
